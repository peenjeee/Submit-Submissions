...
class ApplicationRepositories {
  ...
  async getApplications() {
    const result = await this.pool.query(`${applicationSelect} ORDER BY applications.created_at DESC`);
    return result.rows;
  }
  ...
  async getApplicationsByUser(userId) {
    const query = {
      text: `${applicationSelect} WHERE applications.user_id = $1 ORDER BY applications.created_at DESC`,
      values: [userId],
    };
    const result = await this.pool.query(query);
    return result.rows;
